...
public class MaxiphobicHeap<T extends Comparable<? super T>> implements Heap<T> {
    ...
    */
    ...
    private static <T extends Comparable<? super T>> Tree<T> merge(Tree<T> h1, Tree<T> h2) {
        // TODO
        Tree<T>[] ord = new Tree<>[3];
        Tree<T> ganador = h1.elem.compareTo(h2.elem) >= 0 ? h1 : h2;
        Tree<T> perdedor = h1 == ganador ? h2 : h1;
        ...
        if(h1 == null){
            ...
            ord = ordena(h1.left, h1.right, h2);
            return new Tree<T>(h1.elem, ord[0], merge(ord[1], ord[2]));
        } else {
            ord = ordena(h2.left, h2.right, h1);
            return new Tree<T>(h2.elem, ord[0], merge(ord[1], ord[2]));
        }
    }

    private static <T extends Comparable<? super T>> Tree<T>[] ordena(Tree<T> h1, Tree<T> h2, Tree<T> h3) {
        Tree<T>[] ord = new Tree<>[3];
        if (h2.size <= h1.size && h3.size <= h1.size) {
            ord[0] = h1;
            ord[1] = h2;
            ord[2] = h3;
        } else if (h1.size <= h2.size && h3.size <= h2.size) {
            ord[0] = h2;
            ord[1] = h1;
            ord[2] = h3;
        } else {
