...
union x Empty = x
union Empty y = y
union (Node x n1 s1) (Node y n2 s2) 
   |x < y  = Node x n1 (union s1 (Node y n2 s2))
   |x == y = Node y (n1+n2) (union s1 s2)
   |otherwise = Node y n2 (union s2 (Node x n1 s1))
...
intersection (Node x n xs) (Node y m ys)
   |x == y = Node y ((max n m) - (min n m)) (intersection xs ys)
...
difference Empty y = Empty
difference (Node x n xs) (Node y m ys)
   |x < y = Node x n (difference xs ys)
   |x == y && n-m <= 0 = difference xs ys
   |x == y = Node y (n-m) (difference xs ys)
   |otherwise = Node x n (difference xs ys)
