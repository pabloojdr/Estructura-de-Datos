...
{-
   ...
   Por ejemplo:
                   ...
                   [] -- Lista vacía (no conocemos el tipo que tiene)
   ...
   se hace en orden lexicográfico:
                  ...
                  [5, 7, 9] < [5, 7, 3] -- Se pueden comparar porque son del mismo tipo. Un ejemplo sería (2-1, 'a') == (1, 'a')
...
{-
   ...
   numéricas son de tipo Int.
      ...
      [ (True) ] :: [Bool] / (True) == True / No existen las tuplas unitarias

      ( [True] ) :: [Bool] / Los paréntesis no hacen efecto

      [ (False, '0'), (True, '1') ] :: [(Bool, Char)]

      ( [False, True], ['0', '1'] ) :: ([Bool], String)

      [ [], [2, 3] ] :: [[Int]] / Es una lista de lista de enteros

      [ ["a"] ] :: [[String]] / Es una lista de una lista de una lista de String
...
cuentaVeinte :: [Int]
cuentaVeinte = [0, 20.. 2000]

cuentaImpares :: [Int]
cuentaImpares = [1,3..10000]
...
{-
   ...
   y otras son parciales.
      ...
      null :: [a] -> Bool -- Devuelve True si la lista está vacía

      head :: [a] -> a -- Devuelve el primer elemento de la lista

      last :: [a] -> a -- Devuelve el último elemento de la lista

      tail :: [a] -> [a] -- Devuelve la lista menos el primer elemento

      init :: [a] -> [a] -- Devuelve la lista menos el último elemento

      take :: Int -> [a] -> [a] -- Devuelve los n primeros elementos de la lista

      drop :: Int -> [a] -> [a] -- Devuelve la lista menos los n primeros elementos de la lista
      ...
      elem :: Eq a => a -> [a] -> Bool -- Devuelve True si el elemento se encuentra en la lista

      notElem :: Eq a => a -> [a] -> Bool -- Devuelve True si el elemento NO se encuentra en la lista

      reverse :: [a] -> [a] -- Devuelve la lista en reverso

      (++) :: [a] -> [a] -> [a] -- Suma de dos listas / Si se usa como operador no se escriben paréntesis ([a] ++ [b]) / (++) [a] [b]
-}
...
suma3Mal :: [Int] -> Int
suma3Mal xs = if length xs == 3 then
               head xs + head(tail xs) + last xs
              else
               error "La longitud no es la adecuada"
...
multiplicación :: [Integer] -> Integer
multiplicación [] = 1
multiplicación (x:xs) = x * multiplicación xs
...
longitud' xs = longAc xs 0 -- el 0 es la solución del caso base
   where
      longAc [] ac = ac
      longAc (_:xs) ac = longAc xs (ac+1)
