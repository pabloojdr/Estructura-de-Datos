...
aux g dict stack
   ...
   | not(colored v) = aux g dict' stack'
   | colored v && (cond dict) = Nothing
   | otherwise = aux g dict stack2
-- 
...
--
  where
     ...
     stack' = pushAll stack2 [ (s, nextColor c) | s <- successors g v, not(colored v)]
     cond dict = D.valueOf' v dict /= c
     stack2 = S.pop stack
